function [ zerosave,w,reccalls ] = f_GaussLaguerreQuad( nmax,tol,numpoint,reccalls,maxrec)
%
% function [ zerosave,w,reccalls ] = f_GaussLaguerreQuad( nmax,tol,numpoint,reccalls,maxrec)
%
% This function is aimed at generating the Gauss-Laguerre quadrature nodes
% and weights. This function is tested for N<300; for higher order,
% numerical issues may occur.
%
% This function generates a Gauss-Laguerre quadrature rule for a given n;
% it does so by generating via recursion a table of the Laguerre functions 
...
% interpolating (quadratically) between the two values (or finding an exact 
% zero).
% Using more points will give better accuracy, a warning is output if zeros
% are missed due to lack of points.
% The function will then search the zero until tolerance is reached.
% This tolerance is based on absolute uncertainty in each root.
%
% Standard use of this function:
%
% [x,wx]=f_GaussLaguerreQuad(Nquad)
%
% where x and wx are nodes and weights, Nquad is the desired number of
% quadrature nodes.
%
% Author David Holdaway: year 2012 
%
% Comments by Alberto Tibaldi, 21/07/2014
%
%% check inputs
if nargin==1
...
%     finex = finex( diff(finex)>eps); %if these points are not distinct, remove them
   ...
   [ zerosave,w,reccalls ] = f_GaussLaguerreQuad( nmax,tol,finex,reccalls,maxrec ); 
   return
else
