...
[EigVc,EigVl]=eig(K);
EigVl=diag(EigVl);
[EigVl,indEigVl]=sort(EigVl);

EigVc=EigVc(:,indEigVl);
...
for indPatch=1:length(PatchInfo)
    ...
    f=EigVc(:,indMode).'*PatchInfo(indPatch).coeff_mn.'*PatchInfo(indPatch).f;
